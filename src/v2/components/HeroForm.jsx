...
export default function HeroForm({ hero, updateHero, updateVirtue, addVirtue, removeVirtue }) {
  ...
  const inputClass = "mt-1 block w-full rounded bg-gray-700 border border-gray-600 px-2 py-1.5 text-gray-100 focus:outline-none focus:border-amber-500";
  const sectionHeader = "text-xs font-bold uppercase tracking-widest text-amber-400 mb-3 border-b border-amber-900 pb-1";
    ...
    <div className="space-y-6 text-sm">
          ...
          {hero.virtues.length > 0 ? (
            <>
              ...
              <div className="flex items-stretch gap-1">
                <button
                  type="button"
                  onClick={() => setActiveVirtue((v) => (v === 0 ? hero.virtues.length - 1 : v - 1))}
                ...
                <button
                  type="button"
                  onClick={() => setActiveVirtue((v) => (v === hero.virtues.length - 1 ? 0 : v + 1))}
                  className="shrink-0 rounded bg-gray-700 border border-gray-600 px-1.5 py-1 text-xs text-gray-100 hover:border-amber-500 hover:text-amber-400 transition-colors"
                >
                  ▶
                </button>
              ...
              {virtue && (
                <div className="space-y-2 bg-gray-750 rounded p-2 border border-gray-700">
                  <label className="block">
                    ...
                    <input type="text" value={virtue.name} maxLength={12} onChange={(e) => updateVirtue(clampedActive, 'name', e.target.value)} className={inputClass} />
                  </label>
                  ...
                  {virtue.type === 'champion' && (
                    <label className="block">
                      ...
                      <select value={virtue.kingdom} onChange={(e) => updateVirtue(clampedActive, 'kingdom', e.target.value)} className={inputClass}>
      ...
      <section>
        ...
        <label className="block">
          ...
          <textarea value={hero.description || ''} maxLength={1000} rows={4} onChange={(e) => updateHero('description', e.target.value)} placeholder="Notes about this hero board..." className={inputClass + " resize-none"} />
        </label>
        <label className="block">
          <div className="flex justify-between mb-1">
